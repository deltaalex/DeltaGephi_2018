...
public abstract class AbstractGraph {
    ...
    protected Node createNode(GraphModel graphModel, int i, boolean add) {
        ...
        newNode.getNodeData().setLabel(String.valueOf(i + 1));
        // add to graph
        if (add) {
     ...
     * @param add - Whether to add it to the graph
     */
    protected Node createNode(GraphModel graphModel, String label, boolean add) {
        // create node
        Node newNode = graphModel.factory().newNode();
        // initialize node
        newNode.getNodeData().setSize(NODE_SIZE);
...
//        double x = 10 * random.nextDouble() + 1;
//
//        // raise to inverse square
//        x = Math.pow(x, -2);
//
//        return min + (int) Math.round((max - min) * x);
    ...
    /**
     * Return a random integer index between start (include) and end (exclude)
     * that is different than the given index
     *
     * @param start
     * @param end
     * @param differentThan
    ...
    protected double getEuclideanDistance(Node n1, Node n2) {
        // sqrt( (x1-x2)^2 + (y1-y2)^2 )
        return Math.sqrt(Math.pow(n1.getNodeData().x() - n2.getNodeData().x(), 2)
                + Math.pow(n1.getNodeData().y() - n2.getNodeData().y(), 2));
    }
