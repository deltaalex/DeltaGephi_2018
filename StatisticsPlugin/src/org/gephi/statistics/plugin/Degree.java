...
public class Degree implements Statistics, LongTask {
    ...
    public static final String DEGREE_STR_POWER = "degreestrengthpower";
    ...
    private Map<Integer, Integer> degreePowerDist;
    ...
    public void execute(HierarchicalGraph graph, AttributeModel attributeModel) {
        ...
        degreePowerDist = new HashMap<Integer, Integer>();
        ...
        AttributeColumn degPowCol = graphTable.getColumn(DEGREE_POWER);
        AttributeColumn degStrPowCol = graphTable.getColumn(DEGREE_STR_POWER);
        if (isDirected) {
            if (inCol == null) {
            ...
            if (degPowCol == null) {
                if (nodeTable.hasColumn(DEGREE_POWER)) {
                    degPowCol = nodeTable.getColumn(DEGREE_POWER);
                } else {
                    degPowCol = nodeTable.addColumn(DEGREE_POWER, NbBundle.getMessage(Degree.class, "Degree.nodecolumn.DegreePower"), AttributeType.INT, AttributeOrigin.COMPUTED, 0);
                }
            }
            if (degStrPowCol == null) {
                if (nodeTable.hasColumn(DEGREE_STR_POWER)) {
                    degStrPowCol = nodeTable.getColumn(DEGREE_STR_POWER);
                } else {
                    degStrPowCol = nodeTable.addColumn(DEGREE_STR_POWER, NbBundle.getMessage(Degree.class, "Degree.nodecolumn.DegreeStrPower"), AttributeType.INT, AttributeOrigin.COMPUTED, 0);
                }
            }
        }
        if (degCol == null) {
        ...
        for (Node n : graph.getNodes()) {
            ...
            if (isDirected) {
                ...
                int degPow = inDegree - outDegree;;
                int degStrPow;
                Float strength = (Float) n.getAttributes().getValue("strength");
                if (strength != null) {
                    degStrPow = (int) ((inDegree - outDegree) * 100 * strength);
                } else {
                    degStrPow = 0;
                }
                ...
                row.setValue(degPowCol, degPow);
                row.setValue(degStrPowCol, degStrPow);

                if (!inDegreeDist.containsKey(inDegree)) {
                    inDegreeDist.put(inDegree, 1);
                }
                inDegreeDist.put(inDegree, inDegreeDist.get(inDegree) + 1);
                if (!outDegreeDist.containsKey(outDegree)) {
                    outDegreeDist.put(outDegree, 1);
                }
                outDegreeDist.put(outDegree, outDegreeDist.get(outDegree) + 1);
                if (!degreePowerDist.containsKey(degPow)) {
                    degreePowerDist.put(degPow, 1);
                }
                degreePowerDist.put(degPow, degreePowerDist.get(degPow) + 1);
            }
    ...
    public String getDirectedReport() {
        ...
        XYSeries dpSeries = ChartUtils.createXYSeries(degreePowerDist, "Degree-Power Distribution");
        ...
        ChartUtils.scaleChart(chart2, idSeries, false);
        ...
        ChartUtils.scaleChart(chart3, odSeries, false);
        ...
        JFreeChart chart4 = ChartFactory.createXYLineChart(
                ...
                false);
        chart4.removeLegend();
        ChartUtils.decorateChart(chart4);
        ChartUtils.scaleChart(chart4, dpSeries, false);
        String degreePowerImageFile = ChartUtils.renderChart(chart4, "degree-power-distribution.png");
